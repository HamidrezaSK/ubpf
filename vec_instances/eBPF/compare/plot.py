...
def plot(vanila_jit, modified_jit, aot, x86, cache):
    ...
    for i, (mode, data) in enumerate([ ("vanilla_jit times", vanila_jit), ("x86 times", x86), ("modified_jit times", modified_jit)]):
        ...
        axs[1].bar(x_values + i * bar_width, y_values/min_base, bar_width, label=mode)
        y_values = np.array([max(data[key]) for key in keys])

        axs[2].bar(x_values + i * bar_width, y_values/max_base, bar_width, label=mode)

    # Customize the plot
    axs[0].set_xlabel('Rules Number')
    axs[0].set_ylabel('Execution Time/Vanilla Execution Time')
    ...
    axs[1].set_ylabel('Execution Time/Vanilla Execution Time')
    ...
    axs[2].set_ylabel('Execution Time/Vanilla Execution Time')
