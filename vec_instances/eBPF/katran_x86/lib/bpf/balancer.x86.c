...
#include "katran_x86/lib/linux_includes/bpf.h"
#include "katran_x86/lib/linux_includes/bpf_helpers.h"
#include "katran_x86/lib/linux_includes/jhash.h"

#include "katran_x86/lib/bpf/balancer_consts.h"
#include "katran_x86/lib/bpf/balancer_helpers.h"
#include "katran_x86/lib/bpf/balancer_maps.h"
#include "katran_x86/lib/bpf/balancer_structs.h"
#include "katran_x86/lib/bpf/handle_icmp.h"
#include "katran_x86/lib/bpf/pckt_encap.h"
#include "katran_x86/lib/bpf/pckt_parsing.h"

#define HEAD_SPACE 200
#define TRIAL_NUM 1000000


struct test_map_struct{
...
// static int (*print)(const char,__u64) = (void*)9;
...
__attribute__((__always_inline__)) static inline int process_l3_headers(
    ...
    void* data_end,
    bool is_ipv6,
    struct test_map_struct* my_vip_map) {
  __u64 iph_len;
  int action;
  struct iphdr* iph;
  struct ipv6hdr* ip6h;
...
__attribute__((__always_inline__)) static inline int
process_packet(struct __xdp_md* xdp, __u64 off, bool is_ipv6, struct test_map_struct* my_vip_map) {
  void* data = (void*)(long)xdp->data;
  void* data_end = (void*)(long)xdp->data_end;

  struct ctl_value my_cval = {.mac = {'a','b','c','d','e','f'}};
  struct ctl_value* cval = &my_cval;
  struct real_definition my_dst = {.dst = 0, .dstv6 = {2917007612, 16842442, 0, 33554432}, .flags = 3};
  struct real_definition* dst = &my_dst;
  struct packet_description pckt = {};
  struct vip_definition vip = {};
  struct vip_meta my_vip_info = {.flags=6,.vip_num=1};
  struct vip_meta* vip_info = &my_vip_info;

  struct lb_stats my_data_stats = {.v1=0, .v2=0};
  struct lb_stats* data_stats = &my_data_stats;
  __u64 iph_len;
  __u8 protocol;
  ...
  action = process_l3_headers(
      &pckt, &protocol, off, &pkt_bytes, data, data_end, is_ipv6, my_vip_map);
  if (action >= 0) {
    return action;
  }
  ...
  // vip_info = bpf_map_lookup_elem(&my_vip_map, &vip);
  ...
  if ((vip_info->flags & F_QUIC_VIP)) {
    ...
    if (is_icmp) {
      ...
      struct lb_quic_packets_stats my_quic_packets_stats = {};

      struct lb_quic_packets_stats* quic_packets_stats =
          &my_quic_packets_stats;
    ...
    if (!dst) {
      if (pckt.flow.proto == IPPROTO_TCP) {
        ...
        if (pckt.flags & F_SYN_SET) {
          ...
          // trashing or because another katran_x86 is restarting and all the
...
int balancer_ingress(struct __xdp_md* ctx, struct test_map_struct* my_vip_map) {
  ...
  if (eth_proto == BE_ETH_P_IP) {
    return process_packet(ctx, nh_off, false, my_vip_map);
  } else if (eth_proto == BE_ETH_P_IPV6) {
    return process_packet(ctx, nh_off, true, my_vip_map);
  } else {
...
//   struct __xdp_md ctx = {.data = xdp, .data_end = xdp + xdp_len};
//   struct test_map_struct my_vip_map;
//   int action = balancer_ingress(&ctx,&my_vip_map);
// 	return action;
// }
static void*
...
int main(int argc, char *argv[])
{
    ...
    volatile int ret =0;
    // printf("mem is: %ld\n",mem_len);
    // struct xdp_md* ctx = calloc(1, sizeof(ctx[0])); 
    struct __xdp_md ctx = {.data = mem, .data_end = mem + mem_len};
    struct test_map_struct my_vip_map;
    ...
    for(i = 0; i < TRIAL_NUM ; ++i){
        ret = balancer_ingress(&ctx,&my_vip_map);
    }
